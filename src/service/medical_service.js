...
class MedicalServiceService {
  static async getAll() {
    const [rows] = await db.execute(
      "SELECT * FROM medical_services ORDER BY created_at DESC"
    );
  ...
  static async getById(uuid) {
    const [rows] = await db.execute(
      "SELECT * FROM medical_services WHERE uuid = ?", [uuid]
    );
  ...
  static async create({ name, description, price, specialization_id, clinic_id, hospital_id, image }) {
    ...
    await db.execute(
      `INSERT INTO medical_services 
        (uuid, name, description, price, specialization_id, clinic_id, hospital_id, image, created_at, updated_at) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
  ...
  static async remove(uuid) {
    const [result] = await db.execute(
      "DELETE FROM medical_services WHERE uuid = ?", [uuid]
    );
    return result.affectedRows > 0;
  }
}
