...
class PatientController {
  ...
  static async create(req, res) {
    try {
      const { user_id, name, dob, gender, phone, relationship, address } =
        req.body;
  ...
  static async update(req, res) {
    try {
      const { user_id, name, dob, gender, phone, relationship, address } =
        req.body;
      const updated = await PatientService.update(req.params.id, {
        user_id,
        name,
        dob,
        gender,
  ...
  static async getByUserId(req, res) {
    try {
      ...
      const data = await PatientService.getByUserId(userId);
      res.json({ code: 200, msg: "Thành công", status: "success", data });
    } catch (error) {
      res.status(500).json({ code: 500, msg: error.message, status: "error" });
    }
